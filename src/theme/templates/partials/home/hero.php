<section class="relative h-dvh">
    <div class="absolute top-0 left-0 w-dvw h-dvh max-w-screen-2xl">
        <!-- The post image -->
        <img class="block object-cover object-left w-full h-full brightness-50 saturate-75 contrast-75"
            src="https://plus.unsplash.com/premium_photo-1661761629601-bf9436d058d2?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="test">
    </div>
    <div
        class="absolute top-0 left-0 p-6 pt-24 bg-dark-primary/80 sm:p-12 sm:pt-20 text-light-primary w-dvw h-dvh max-w-screen-2xl">
        <div class="flex flex-col items-center justify-center w-full h-full">
            <div class="flex flex-col justify-end w-full h-1/2">
                <strong class="pb-2 font-mono text-xl italic text-light-primary"><span class="text-brand-dark">//</span>
                    Featured article</strong>
                <h1 class="max-w-3xl text-5xl leading-tight text-brand-light">How to build a home server for the
                    rest of
                    us
                </h1>
            </div>
            <div class="flex items-end w-full h-1/2">
                <ul class="flex items-center w-full text-lg sm:w-3/4">
                    <li class="flex flex-col pr-16">
                        <strong class="font-mono italic text-brand-light">Published</strong>
                        <span class="text-base">July 31st 2024</span>
                    </li>
                    <li class="flex flex-col pr-16">
                        <strong class="font-mono italic text-brand-light">Author</strong>
                        <span class="text-base">@huntertrammell</span>
                    </li>
                    <li class="flex flex-col pr-16">
                        <strong class="font-mono italic text-brand-light">Reading time</strong>
                        <span class="text-base">3 minutes</span>
                    </li>
                    <li class="flex flex-col pr-16">
                        <strong class="font-mono italic text-brand-light">Tags</strong>
                        <span class="text-base"><u>#coding</u> <u>#coding</u> <u>#coding</u> </span>
                    </li>
                </ul>
                <button
                    class="w-16 h-16 p-3 ml-auto text-center bg-gradient-to-br from-brand-dark via-brand-primary to-brand-light text-light-primary">
                    <?php get_template_part('assets/icons/arrow-b-r'); ?>
                </button>
            </div>
        </div>
    </div>
</section><!-- #featured -->